import type { OrdenCompra, OrdenDePedidoProducto, Product } from "$lib/interfaces/tipos";



const detail = (productos:OrdenDePedidoProducto[]) => {
    const chain = productos.reduce((acum,detalle) => {
      console.log(JSON.stringify(detalle.Product.tax,null,2));
      return acum  + `
      <div style="margin-top: 16px; margin-bottom: 16px; background-color: #f8fafc; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -2px rgba(0, 0, 0, 0.1)">
        <p style="text-align: center; font-size: 18px; font-weight: 600; color: blue">${detalle.Product.name}</p>
        <div style="margin-left: 40px; margin-right: 40px; font-size: 18px;">
          
        <div>
            <p><span style="font-weight: 600;">Valor unit: </span> ${(detalle.precio / (1+detalle.Product.tax/100)).toLocaleString('es-ES', { style: 'currency', currency: 'COP' })}
            </p>
        </div>

        <div>
            <p><span style="font-weight: 600" ;>Iva       : </span> ${(detalle.precio / (1+detalle.Product.tax/100)).toLocaleString('es-ES', { style: 'currency', currency: 'COP' })}
            </p>
        </div>
          ...
          <div>
            <p><span style="font-weight: 600;">Total    : </span> ${(detalle.precio * detalle.cantidad).toLocaleString('es-ES', { style: 'currency', currency: 'COP' })}
            </p>
          </div>
        </div>
      </div>`
    ...
    export const htmlTable = (orden:OrdenCompra) => {
      const detalle = detail(orden.OrdenDePedidoProducto);
      const table = `
      <!DOCTYPE html>
